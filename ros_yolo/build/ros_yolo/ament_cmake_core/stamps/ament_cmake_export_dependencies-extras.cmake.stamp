...
set(_exported_dependencies "builtin_interfaces;std_msgs;sensor_msgs;builtin_interfaces;std_msgs;sensor_msgs;geometry_msgs;rosidl_runtime_c;rosidl_typesupport_interface;builtin_interfaces;std_msgs;sensor_msgs;geometry_msgs;builtin_interfaces;std_msgs;sensor_msgs;geometry_msgs;rosidl_default_runtime;sensor_msgs;std_msgs;rclcpp;rclpy")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to ros_yolo_DEFINITIONS, ros_yolo_INCLUDE_DIRS,
# ros_yolo_LIBRARIES, and ros_yolo_LINK_FLAGS.
# Additionally collect the direct dependency names in
# ros_yolo_DEPENDENCIES as well as the recursive dependency names
# in ros_yolo_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(ros_yolo_DEPENDENCIES ${_exported_dependencies})
  set(ros_yolo_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(ros_yolo_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(ros_yolo_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(ros_yolo_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(ros_yolo_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(ros_yolo_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND ros_yolo_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
